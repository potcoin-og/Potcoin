...
*Setup instructions for a gitian build of Potcoin using a Debian VM or physical system.*

Gitian is the deterministic build process that is used to build the Potcoin
...
- [Building Potcoin](#building-potcoin)
...
Clone the git repositories for potcoin and gitian,
...
In [doc/release-process.md](release-process.md) in the potcoin repository under 'Fetch and build inputs'.
...
To build Potcoin (for Linux and/or Windows) just follow the steps under 'perform
gitian builds' in [doc/release-process.md](release-process.md) in the potcoin repository.

Output from `gbuild` will look something like

    Initialized empty Git repository in /home/debian/gitian-builder/inputs/potcoin/.git/
    remote: Reusing existing pack: 35606, done.
    remote: Total 35606 (delta 0), reused 0 (delta 0)
    Receiving objects: 100% (35606/35606), 26.52 MiB | 4.28 MiB/s, done.
    Resolving deltas: 100% (25724/25724), done.
    From https://github.com/potcoin/potcoin
...
For example:
```bash
URL=https://github.com/user/potcoin.git
COMMIT=2014_03_windows_unicode_path
./bin/gbuild --commit potcoin=${COMMIT} --url potcoin=${URL} ../potcoin/contrib/gitian-descriptors/gitian-linux.yml
./bin/gbuild --commit bitcoin=${COMMIT} --url potcoin=${URL} ../potcoin/contrib/gitian-descriptors/gitian-win.yml
```
...
```bash
    gpg --detach-sign ${VERSION}/${SIGNER}/potcoin-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/potcoin-build.assert
```
...
[potcoin/gitian.sigs](https://github.com/potcoin/gitian.sigs/) repository, or
